...
| AI image generation failure | Default blocks at the recovery decision; Quick does not | Retry the failed item once through the same GlenClaw manifest path. Default then asks once to repair the runtime and retry or cancel/replan affected AI images. Quick removes exhausted AI/dependent-slice jobs, replans them with native editable text/SVG or already prepared non-AI assets, and continues | Default chooses repair/retry or cancel/replan | Default: Step 5 decision → same path or Step 4 affected-plan repair; Quick: current resource plan |
