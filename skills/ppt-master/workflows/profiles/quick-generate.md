...
| Image-to-PPTX reconstruction asset | In Axion, preserve identity graphics through an exact vector, deterministic redraw, sufficient source asset, or GlenClaw reference-based high-resolution reconstruction; keep data graphics native-and-verified or exact. For scene imagery, build the minimum registered clean-base/midground/subject/foreground group; batch padded-bbox-disjoint objects into one shared plate, then split them with grid slicing or independent nested-SVG bbox crops |
...
**Quick exhausted-automation no-AI replan**: Follow [`image-generator.md`](../../references/image-generator.md) §7 when GlenClaw generation or its required dependent slicing is exhausted: ask no path question, remove the affected AI jobs and stale manifest entries, preserve their communication content with native editable text/SVG or already prepared non-AI assets, and continue the same run. To retain AI imagery after failure, repair the GlenClaw runtime and start a new Quick run.
